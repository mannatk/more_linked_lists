...
public class DNALinkedList 
{
	...
	private boolean matches(Node<Character> startNode, String target)
	{
		...
		for(int i=0;i<target.length();i++){
			if(current==null || current.getData()!=target.charAt(i))
				return false;
	...
	public void transpose(String transposon, String target)
	{
		...
		Node<Character> n = firstNodeOfTransposon;
		for (int i = 0; i < transposon.length()-1; i++) {
			n= n.getNext();
		}
		Node<Character> lastNodeOfTransposon= n;		
		...
		System.out.println("reverse"+transposonList);
	...
	public static void main(String[] args) 
	{
		...
		System.out.print("transposed: " + list);
	}
}
